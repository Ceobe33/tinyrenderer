 ...
 *  - Note:
 *      Runtime behaviour may differ from debug behaviour, as the compiler may
 * optimizer code. Debug mode should be used, and one should be aware of the
 * compiler's optimisation settings.
 *
 * : Zach Wolpe
...
class ProfilerInterface {
   public:
    static void makdir(const std::string& sav_loc = "temp-store") {
        ...
        if (rc == 0)
            std::cout << "Directory created successfully: " << ss.str()
                      << std::endl;
        else
...
class Instrumentor {
   ...
   public:
    ...
    void LaunchSession(const std::string& name,
    ...
    void WriteProfile(const ProfileResult& result) {
        // std::cout << " >> Writing profile (ProfileCount = " << ProfileCount
        // << ")" << std::endl;
        ...
        OutputStream << "}";
        OutputStream.flush();  // flushes the stream buffer, to ensure the data
                               // is written to the file immediately. So that we
                               // do not lose data if the program crashes.
    }
...
class CPUProfiler : public ProfilerInterface {
   ...
   public:
    ...
    void cpu_EndSession() {
        ...
        std::cout << " >> CPUProfiler: " << name << " took " << ms << "ms ("
                  << s << "s)" << std::endl;
